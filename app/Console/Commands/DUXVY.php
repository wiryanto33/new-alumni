...
class DUXVY extends Command
{
     ...
     * @var string
     */
    protected $signature = 'duxvy {--lqs=} {--v=}';
    ...
    public function handle()
    {
        ...
        try {
            ...
            $dbBuildVersion = getCustomerAddonBuildVersion('ALUDONATION');
            $v = $this->option('v');

            $logger->log('Command duxvy', '===========START==========');
            ...
            if ($dbBuildVersion < $v) {
                ...
                setCustomerAddonBuildVersion('ALUDONATION', $v);
                DB::commit();
                $logger->log('Command RUN', 'DONE');
                $logger->log('Command duxvy', '===========END==========');
                return true;
            } else {
                DB::rollBack();
