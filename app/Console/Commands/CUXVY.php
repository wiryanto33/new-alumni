...
class CUXVY extends Command
{
     ...
     * @var string
     */
    protected $signature = 'cuxvy {--lqs=} {--v=}';
    ...
    public function handle()
    {
        ...
        try {
            ...
            $dbBuildVersion = getCustomerAddonBuildVersion('ALUCOMMITTEE');
            $v = $this->option('v');

            $logger->log('Command cuxvy', '===========START==========');
            ...
            if ($dbBuildVersion < $v) {
                ...
                setCustomerAddonBuildVersion('ALUCOMMITTEE', $v);
                DB::commit();
                $logger->log('Command RUN', 'DONE');
                $logger->log('Command cuxvy', '===========END==========');
                return true;
            } else {
                DB::rollBack();
