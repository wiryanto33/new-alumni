                ...
                <div class="primary-form">
                    ...
                    <div class="pb-40">
                        ...
                        <h4 class="fs-16 fw-400 lh-25">{{ __('Already have an account?') }} <a href="{{ route('login') }}"
                                class="text-decoration-underline fw-500 text-black hover-color-one">{{ __('Sign In') }}</a>
                        </h4>
                    </div>
                    <!-- Form -->
                    <form action="{{ route('register') }}" enctype="multipart/form-data" method="post">
                        ...
                        <div class="form-wrap-main pb-25">
                            ...
                            <div class="primary-form-group">
                                <div class="primary-form-group-wrap">
                                    <label for="rank" class="form-label">{{ __('Rank') }}<span
                                            class="text-danger"> *</span></label>
                                    <input type="text" class="primary-form-control" id="fullName"
                                        value="{{ old('rank') }}" name="rank" placeholder="{{ __('Pangkat') }}" />
                                </div>
                                @error('rank')
                                    <span class="fs-12 text-danger">{{ $message }}</span>
                                @enderror
                            ...
                            <div class="primary-form-group">
                                <div class="primary-form-group-wrap">
                                    <label for="nrp" class="form-label">{{ __('Nrp') }}<span
                                            class="text-danger"> *</span></label>
                                    <input type="text" class="primary-form-control" id="fullName"
                                        value="{{ old('nrp') }}" name="nrp" placeholder="{{ __('Nrp') }}" />
                                </div>
                                @error('nrp')
                                    <span class="fs-12 text-danger">{{ $message }}</span>
                                @enderror
                            ...
                            @if (!isCentralDomain() || !isAddonInstalled('ALUSAAS'))
                                            ...
                                            <select class="primary-form-control sf-select-without-search" id="Department"
                                                ...
                                                @foreach ($departments as $department)
                                                    <option {{ old('department_id') == $department->id ? 'selected' : '' }}
                                                        value="{{ $department->id }}">{{ __($department->short_name) }}
                                                    </option>
                                                @endforeach
                                            </select>
                                        </div>
                                ...
                                @if ($regForm->enable_passing_year)
                                    <div class="primary-form-group">
                                        <div class="primary-form-group-wrap">
                                            <label for="PassingYear" class="form-label">{{ __('Passing Year') }}<span
                                                    class="text-danger">
                                                    *</span></label>
                                            <select class="primary-form-control sf-select-without-search" id="PassingYear"
                                                ...
                                                @foreach ($passingYears as $passingYear)
                                                    <option
                                                        {{ old('passing_year_id') == $passingYear->id ? 'selected' : '' }}
                                                        value="{{ $passingYear->id }}">{{ __($passingYear->name) }}
                                                    </option>
                                                @endforeach
                                            </select>
                                        </div>
    ...
    @if (!isCentralDomain() || !isAddonInstalled('ALUSAAS'))
        <script>
            ...
            $(document).on('click', '#reg-form-btn', function() {
                $(this).closest('form').find('input[name=custom_fields]').val(JSON.stringify(formRenderInstance
                    .userData))
            });
        </script>
    @endif
