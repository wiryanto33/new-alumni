(function ($) {
    ...
    var committee_designation_id;
    var committeeVoteDatatable = $('#committeeVoteDatatable').DataTable({
        pageLength: 10,
        ordering: false,
        serverSide: true,
        processing: true,
        ...
        responsive: true,
        ajax: {
            url: $('#pending-candidate-route').val(),
            data: function (d) {
                d.committee_election_id = election_id;
                d.committee_category_id = committee_category_id;
                d.committee_designation_id = committee_designation_id;
            }
        },
        language: {
            paginate: {
    ...
    $(document).on('change', '#election_id,#committee_id,#position_id', function (e) {
        election_id = $(document).find('#election_id :selected').val();
        committee_category_id = $(document).find('#committee_id :selected').val();
        committee_designation_id = $(document).find('#position_id :selected').val();
        committeeVoteDatatable.draw();
        e.preventDefault();
    });
    ...
    $(document).on('click', '.statusChange', function (e) {
        ...
        $('#status-change-modal').find('input[name=id]').val(id);
        $('#status-change-modal').find(':input[name=status]').val(status);
        $('#status-change-modal').find('.sf-select-without-search').niceSelect('update');
        $('#status-change-modal').modal('toggle')
    });
