(function ($) {
    ...
    $(document).on('change', '.change_status-ss', function(){
        ...
        var targetUrl = $('#donation-update-route').val();
            ...
            if (result.value) {
                $.ajax({
                    ...
                    success: function (response) {
                        ...
                        $('#pendingDonationDataTable').DataTable().ajax.reload();
                    },
    ...
    $("#pendingDonationDataTable").DataTable({
        pageLength: 10,
        ordering: false,
        serverSide: true,
        responsive: true,
        processing: true,
        searching: true,
        ajax: $('#pending-donation-route').val(),
        language: {
            paginate: {
        ...
        columns: [
            { "data": "name", "name": "campaign_donations.name", responsivePriority: 1 },
            { "data": "email", "name": "campaign_donations.email", responsivePriority: 1 },
